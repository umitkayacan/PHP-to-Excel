<?php
	...
	if ($veriler=$db->get_results("SELECT * FROM tbl_uyeler"))
	{
		$columns=array();
		$data=array();
		$replaceDotCol=array(0); 
		$columns=array(
			'Adı Soyadı',
			'KullanıcıAdı'
		);
		...
		{			
			$data[]=array($veri->adisoyadi,$veri->kullaniciadi );
		}
		exportExcel('Üyeler',$columns,$data,$replaceDotCol);
	}
